local o = {
    ...
    log_path = "/var/log/mpv/history.log",
...
function display_list()
	...
	mp.add_forced_key_binding("ctrl+n", "recent-UP", function()
        start, choice = select(list, start, choice, -1)
    end, {repeatable=true})
	--mp.add_forced_key_binding("DOWN", "recent-DOWN", function()
	mp.add_forced_key_binding("ctrl+p", "recent-DOWN", function()
        start, choice = select(list, start, choice, 1)
    end, {repeatable=true})
	...
	mp.add_forced_key_binding("ctrl+[", "recent-ESC", function() unbind() end)
end
